<div class="flex flex-col h-auto min-h-screen px-2 py-4 pr-4 bg-white border-l lg:w-6/12 dark:border-gray-700 dark:bg-gray-800">
    <div>
        <ul
            class="items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            @if (restaurant()->allow_dine_in_orders)
                <li
                    class="w-full border-b border-gray-200 cursor-pointer sm:border-b-0 sm:border-r dark:border-gray-600">
        ...
        <div class="flex items-center justify-between mt-1 mb-2">
            <div class="inline-flex items-center gap-1 py-2 font-medium dark:text-neutral-200">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="w-6 h-6 bi bi-receipt" viewBox="0 0 16 16">
        ...
        <div class="flex items-center justify-between gap-2">
            @if ($orderType == 'dine_in')
                ...
                @elseif($selectWaiter)
                    <div class="inline-flex items-center gap-1 py-2 text-sm dark:text-gray-300">
                        <svg class="w-5 h-5 text-gray-600 dark:text-gray-300" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2m0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3m0 14.2a7.2 7.2 0 0 1-6-3.22c.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08a7.2 7.2 0 0 1-6 3.22"/></svg>
            ...
            @if ($orderType == 'delivery' && user_can('Update Order'))
                <div class="flex items-center justify-between gap-2">
                    <div class="inline-flex items-center gap-2">
                        ...
                        <x-select class="w-full text-sm" wire:model.live='selectDeliveryExecutive' wire:change='saveDeliveryExecutive'>
                ...
                <table class="flex-1 min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" wire:key='menu-item-list-{{ microtime() }}'>
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" wire:key='menu-item-{{ $key . microtime() }}' wire:loading.class.delay='opacity-10'>
                            ...
                            <td class="p-2 text-base text-center text-gray-900 whitespace-nowrap">
                        ...
                        @if (!is_null($kot->note))
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td class="p-2 text-xs italic whitespace-normal dark:text-white" colspan="5">
                                    {{ $kot->note }}
                                </td>
                            </tr>
                        @endif
        ...
        <div class="w-full h-auto p-4 mt-3 space-y-4 text-center rounded select-none bg-gray-50 dark:bg-gray-700">
            ...
            <div wire:key="discountAmount" class="flex justify-between text-sm text-green-500 dark:text-green-400">
                ...
                <div class="inline-flex items-center gap-x-1">@lang('modules.order.discount') @if ($discountType == 'percent') ({{ $discountValue }}%) @endif
                    <span class="text-red-500 cursor-pointer hover:scale-110 active:scale-100" wire:click="removeCurrentDiscount">
        ...
        <div class="w-full h-auto pt-3 pb-4 select-none">
            @if ($orderDetail->status == 'kot' && user_can('Update Order'))
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <button class="w-full p-2 text-white bg-green-500 rounded" wire:click="saveOrder('bill', 'payment')">
                    ...
                    <a href="{{ route('pos.kot', ['id' => $orderDetail->id]) }}" class="w-full p-2 text-center bg-white border rounded text-skin-base border-skin-base">
                        @lang('modules.order.newKot')
                    </a>

                    @if (user_can('Delete Order'))
                    <button class="w-full p-2 text-white bg-red-500 rounded" wire:click="saveOrder('cancel')">
