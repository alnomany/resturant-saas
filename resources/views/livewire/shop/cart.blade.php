<div>
    <section class="hidden px-4 bg-white dark:bg-gray-900 lg:block" style="display:none;">
        <div
    ...
    <div class="flex flex-col px-4 my-4" x-data="{ showAll: false }">
        ...
        @if (count($menuList) > 8)
            <div class="flex justify-center mt-4" x-cloak wire:key="show-more-button">
                <button @click="showAll = !showAll" class="flex items-center gap-1 text-sm text-skin-base hover:underline">
                    <span x-text="showAll ? '{{ __('modules.menu.showLess') }}' : '{{ __('modules.menu.showMore') }}'"></span>
                    <svg class="w-4 h-4 transition-transform duration-200" :class="{'rotate-180': showAll}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
            </div>
        @endif
    </div>
        ...
        <div class="mx-4 mt-6">
                ...
                <div x-show="open" x-cloak
                    ...
                    <div class="overflow-y-auto max-h-80">
                        ...
                        <button wire:click="filterMenu({{ $item->id }}); $nextTick(() => { open = false })"
                            ...
                            <span class="px-2 py-1 text-xs text-gray-600 bg-gray-100 rounded-full dark:bg-gray-600 dark:text-gray-300">
                                {{ $item->items->count() }}
                            </span>
                        </button>
                        @endif
            ...
            <div class="hidden p-2 rounded-md lg:block group bg-gray-50 dark:bg-gray-800">
                <nav class="flex gap-2 overflow-x-auto group-hover:[&::-webkit-scrollbar-thumb]:bg-gray-300 dark:group-hover:[&::-webkit-scrollbar-thumb]:bg-gray-600 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-track]:hidden [&::-webkit-scrollbar-thumb]:rounded-full py-2" aria-label="Categories">
                    <button wire:click="filterMenu(null)"
                        @class([
                            'px-4 py-2 text-sm font-medium rounded-lg transition-colors whitespace-nowrap',
                            'bg-skin-base text-white shadow-sm' => is_null($filterCategories),
                            'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' => !is_null($filterCategories),
                        ])>
                        @lang('app.showAll')
                    </button>
                    ...
                    @foreach ($categoryList as $item)
                        ...
                        <button wire:click="filterMenu({{ $item->id }})"
                            @class([
                                ...
                                'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' => $filterCategories != $item->id,
                            ])>
                            <span>{{ $item->getTranslation('category_name', session('locale', app()->getLocale())) }}</span>
                            <span class="px-2 py-0.5 text-xs rounded-full {{ $filterCategories == $item->id ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300' }}">
                                {{ $item->items->count() }}
                            </span>
                        </button>
                        @endif
        ...
        <div class="grid grid-cols-3 mx-4 my-6">
            ...
            <div class="flex items-center justify-end" wire:key="veg_button">
                <label class="inline-grid w-12 h-6 bg-gray-300 rounded-full shadow cursor-pointer peer-checked:bg-green-500" id="veg_toggle">
                    <input type="checkbox" value="1" wire:model.live='showVeg' class="sr-only peer">
                    <span class="w-6 h-6 transition-transform duration-300 bg-white rounded-full shadow peer-checked:bg-green-500 rtl:peer-checked:-translate-x-full ltr:peer-checked:translate-x-full"></span>
                </label>
                <span class="text-sm font-medium text-gray-900 ms-3 dark:text-gray-300">@lang('modules.menu.typeVeg')</span>
            </div>
...
@endif
    ...
    @if ($showCart)
        ...
        @if ($restaurant->allow_customer_orders)
            <div class="flex my-4">
                ...
                <ul class="flex items-center w-full mx-4 text-sm font-medium text-gray-900 bg-white border border-gray-200 divide-x rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
        ...
        <div class="px-4 mt-4 space-y-4">
            @foreach ($orderItemList as $key => $item)
                <div class="flex items-center justify-between gap-6 transition bg-white border rounded-lg shadow-sm hover:shadow-md dark:border-gray-600 dark:lg:bg-gray-900 dark:shadow-sm" wire:key='menu-item-{{ $item->id . microtime() }}'>
                    <div class="flex w-full p-4 space-x-4 dark:bg-gray-800 dark:text-gray-200">
                        <!-- Item Image -->
                        <img class="object-cover w-10 h-10 rounded-lg cursor-pointer lg:w-16 lg:h-16" wire:click="showItemDetail({{ $item->id }})" src="{{ $item->item_photo_url }}" alt="{{ $item->item_name }}">

                        <!-- Item Details -->
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-col items-start justify-between w-full gap-2 sm:flex-row sm:items-baseline">
                                ...
                                <div class="flex flex-wrap items-center justify-between gap-3 sm:w-auto md:w-1/3">
                                    ...
                                    <div class="flex items-center">
                                        <button type="button"
                                                ...
                                                <svg class="w-2 h-2 text-gray-900 dark:text-white" aria-hidden="true" viewBox="0 0 18 2"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 1h16"/></svg>
                                        </button>
                                        ...
                                        <button type="button"
                                                ...
                                                <svg class="w-2 h-2 text-gray-900 dark:text-white" aria-hidden="true" viewBox="0 0 18 18"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 1v16M1 9h16"/></svg>
                                        </button>
                                    </div>
                            ...
                            <div class="mt-2">
                                @if(isset($this->itemNotes[$key]) && !empty($this->itemNotes[$key]))
                                    <div class="flex items-center mt-2">
                                        <span class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-600 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300">
                                            <svg class="w-3.5 h-3.5 mr-1.5" viewBox="0 0 24 24" stroke="currentColor" fill="none"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8-4-4H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-3z"/></svg>
                                            <span class="mr-1.5">{{ $this->itemNotes[$key] }}</span>
                                            <button
                                                wire:click="$set('itemNotes.{{ $key }}', '')"
                                ...
                                @else
                                    <div x-data="{ showNoteInput: false, noteText: '' }" class="mt-2">
                                        ...
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" class="w-3.5 h-3.5 group-hover:scale-110 transition-transform duration-200" xml:space="preserve"><path d="M11.3 26.5 4 28l1.5-7.3L21.6 4.5c.8-.8 2.1-.8 2.9 0l2.9 2.9c.8.8.8 2.1 0 2.9zm7.4-19 5.8 5.8m-5.8 0-8.8 8.8" style="fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10"/></svg>
                                        ...
                                        <div
                                            ...
                                            x-transition:enter-end="opacity-100 transform scale-100" class="flex items-center mt-2"
                                        >
                                                ...
                                                <div class="relative flex-1">
                                                    ...
                                                    <div class="absolute inset-y-0 right-0 flex items-center gap-1 pr-2">
                                                        <button
                                                            ...
                                                            title="@lang('app.save')">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m5 13 4 4L19 7"/></svg>
                                                        </button>
                                                        <button
                                                            ...
                                                            <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18 18 6M6 6l12 12"/></svg>
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <div>
                    <div
                            ...
                            <div x-data="{ showNotes: false }" x-cloak>
                                ...
                                <div x-show="showNotes"
                                    ...
                                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <!-- Form Content -->
                                        <div class="p-3">
                                            <x-textarea id="orderNote" class="block w-full mt-1" rows="3" wire:model.live.debounce.750ms="orderNote"  placeholder="{{ __('placeholders.addOrderNotesPlaceholder') }}"/>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div x-show="!showNotes && $wire.orderNote"
                                    ...
                                    class="mt-3">
                                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700/50 dark:border-gray-600">
                                        <!-- Note Icon & Text -->
                                        <div class="flex items-center gap-3">
                        ...
                        @if (count($orderItemList) > 0 && $extraCharges)
                            ...
                            <div wire:key="extraCharge-{{ $loop->index }}" class="flex justify-between text-sm text-gray-500 dark:text-neutral-400">
                    ...
                    @if ($orderType === 'delivery' && !empty($deliveryAddress))
                        <div class="w-full h-auto p-4 mt-3 rounded select-none bg-gray-50 dark:bg-gray-700">
                            ...
                            @if (!empty($deliveryAddress))
                                <div class="p-3 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="w-full h-auto pt-3 pb-4 text-center select-none" wire:key='order-{{ microtime() }}'>
                        <div class="flex gap-2">
                            ...
                            @else
                                <div class="grid w-full grid-cols-2 gap-2">
                                    ...
                                    @if (!$order)
                                        @if ($showPayNow)
                                            <x-button class="flex items-center justify-center w-full gap-2" wire:click="placeOrder(true)" wire:loading.delay.attr="disabled">
                                                <span wire:loading.delay wire:target="placeOrder(true)">{!! $loadingSpinner !!}</span>
                                                @lang('modules.order.payNow')
                                            </x-button>

                                            @if (!$isPaymentEnabled)
                                                <x-secondary-button class="flex items-center justify-center w-full gap-2" wire:click="placeOrder" wire:loading.delay.attr="disabled">
                                                    <span wire:loading.delay wire:target="placeOrder">{!! $loadingSpinner !!}</span>
                                                    @lang('modules.order.payLater')
                                                </x-secondary-button>
                                            @endif
                                        @else
                                            <x-button class="flex items-center justify-center w-full gap-2" wire:click="placeOrder" wire:loading.delay.attr="disabled">
                                                <span wire:loading.delay wire:target="placeOrder">{!! $loadingSpinner !!}</span>
        ...
        <x-slot name="content">
            @if ($selectedItem)
                <div class="flex flex-col gap-2">
                    <div class="flex flex-col gap-2">
                        <img src="{{ $selectedItem->item_photo_url }}" alt="{{ $selectedItem->item_name }}" class="object-cover w-full rounded-md">
                        <div class="flex flex-col gap-1">
                            ...
                            @if (strlen($selectedItem->description) > 100)
                                <div x-data="{ expanded: false }">
                                    ...
                                    <button @click="expanded = !expanded" class="mt-1 text-sm font-medium text-skin-base">
    ...
    @if ($paymentOrder)
        <x-dialog-modal wire:model.live="showPaymentModal" maxWidth="md">
            ...
            <x-slot name="content">
                <div class="flex items-center justify-between p-2 mb-6 rounded-md cursor-pointer bg-gray-50 dark:bg-gray-800">
                                ...
                                <x-secondary-button wire:click='initiatePayfastPayment({{  $paymentOrder->id }})'>
                                    <span class="inline-flex items-center">
                                        <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8 6 L14 12 L8 18" fill="none" stroke="#E63950" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
