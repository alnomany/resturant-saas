...
class Pos extends Component
{
    ...
    public function showPayment($id)
    {
        ...
        $this->dispatch('showPaymentModal', data: ['id' => $order->id]);
    }
        ...
        if ($status == 'kot') {
            ...
            if ($this->orderID) {
                ...
                foreach ($this->taxes as $tax) {
                    $this->total += (($tax->tax_percent / 100) * $this->discountedTotal);
                }
    ...
    // loyaltyservice ahmed added
    public function updatedCustomerPhone()
    {
        $this->customer = Customer::where('phone', $this->customerPhone)->first();

        if ($this->customer) {
            $this->customerPoints = $this->customer->loyalty_points;

            $this->availableDiscount = app(LoyaltyService::class)
                ->calculateDiscount($this->customer, $this->cartTotal);
    ...
    {
        $discount = $this->usePoints ? $this->availableDiscount : 0;

        $this->pointsUsed = $this->usePoints
            ? floor($discount / 10) * 100
            : 0;

        $this->pointsEarned = app(LoyaltyService::class)
            ->calculateEarnedPoints($this->cartTotal);

        $this->finalAmount = $this->cartTotal - $discount;
    }
