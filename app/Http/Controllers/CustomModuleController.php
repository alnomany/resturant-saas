...
class CustomModuleController extends Controller
{
    ...
    public function validateModule($moduleName)
    {
        ...
        if (!isset($config['parent_min_version'])) {
            $errorMessage = App::environment('local') ? 'Please download and install the latest version of the module.' : 'Minimum version of <b>' . $appName . ' main application</b> is not defined in the Module.';
    ...
    public function update(Request $request, $moduleName)
    {
        ...
        // /** @phpstan-ignore-next-line */
        $plugins = \Nwidart\Modules\Facades\Module::allEnabled(true);
        ...
        return Reply::redirect(route('superadmin.custom-modules.index') . '?tab=custom', 'Status Changed. Reloading');
    }
    ...
    private function runActivateCommand($moduleName)
    {
        ...
        if (Arr::has($artisanCommands, $command)) {
